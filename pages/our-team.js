...
import OurTeamTable from '../components/tableOurTeam/index'
            ...
            <div className="pad-">
                <div className="tab-content" id="myTabContent">
                    <div className="tab-pane fade show active" id="mei" role="tabpanel" aria-labelledby="mei-tab">
                        <OurTeamTable dataOurTeam={dataOurTeam} />
                    </div>
                </div>
                <style jsx>{`
                        .pad-1{
...
OurTeam.getInitialProps = async (ctx) => {
    ...
    if(!token && token !== undefined && ctx.pathname === '/our-team'){
        if(process.browser){
            Router.push('/login')
        }
    }

    return { dataOurTeam: json }
}
